...
export type Knot = {
  ...
  /** 대략적인 강도 유지율. 줄 종류·조임 상태에 따라 달라질 수 있음 */
  strength?: string;
  /** 낚시 정보 허브 추천 노출 */
  featured?: boolean;
  /** 검색·필터용 키워드 */
...
export const KNOT_DIFFICULTY_ORDER: Record<KnotDifficulty, number> = {
  'very-easy': 0,
  easy: 1,
  medium: 2,
  hard: 3,
...
export const KNOT_SORT_OPTIONS = [
  { key: 'name', label: '이름순' },
  { key: 'difficulty-asc', label: '쉬운 순' },
  { key: 'difficulty-desc', label: '어려운 순' },
] as const;

export type KnotSort = (typeof KNOT_SORT_OPTIONS)[number]['key'];

export const KNOTS_DISCLAIMER =
  '매듭 강도는 줄 종류·굵기·조임 상태에 따라 달라집니다. 중요한 출조 전에는 충분히 연습하고, 매듭 상태를 여러 번 확인하세요.';
...
export const KNOTS: Knot[] = [
  {
    ...
    difficulty: 'easy',
    strength: '줄 강도 약 95% 유지(조건에 따라 다름)',
    featured: true,
    tags: ['루어', '바늘', '스위벨', 'PE', '모노'],
    steps: [
      '낚싯줄을 15~20cm 정도 접어 이중 고리를 만든다.',
      '이중 고리를 스위벨·바늘·루어 눈에 통과시킨다.',
      '줄 끝으로 느슨한 오버핸드 매듭을 만든 뒤, 루어(또는 바늘) 전체를 그 고리 안으로 통과시킨다.',
      '줄을 천천히 당겨 매듭이 형태를 잡게 한 뒤, 여분 줄을 잘라낸다.',
    ],
    tips: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    strength: '줄 강도 약 85~90% 유지(조건에 따라 다름)',
    tags: ['범용', '스냅', '루어'],
    steps: [
    ...
    tips: [
      '모노·플루오로는 5~6회, PE는 7~8회 감기면 안정적이다.',
  ...
  },
  {
    ...
    strength: '줄 강도 약 90% 유지(조건에 따라 다름)',
    featured: true,
    tags: ['초보', '바늘', '모노'],
    steps: [
      ...
      '줄 끝을 다시 큰 고리(본줄과 감긴 줄 사이)에 한 번 더 통과시킨다.',
      '본줄과 줄 끝을 동시에 당겨 조인다.',
    ],
    tips: [
      '줄 끝을 15cm 이상 남기면 작업이 편하다.',
      'PE 라인보다 모노·플루오로에 더 잘 맞는다.',
    ],
  },
  {
    slug: 'snell',
    ...
    category: 'terminal',
    icon: '🪡',
    summary: '바늘 자루에 줄을 감아 고정하는 터미널 매듭',
    difficulty: 'medium',
    strength: '줄 강도 약 90~95% 유지(조건에 따라 다름)',
    tags: ['바늘', '원투', '바다'],
    steps: [
      '줄 끝을 바늘 눈에 통과시켜 자루를 따라 아래로 내린다.',
      '줄 끝으로 바늘 자루와 본줄을 함께 5~7회 감는다.',
      '감은 줄 끝을 다시 바늘 눈 쪽으로 빼낸다.',
      '본줄을 당겨 감긴 부분이 자루에 밀착되도록 조인다.',
      '여분 줄을 짧게 잘라낸다.',
    ],
    tips: [
      '바늘 끝이 라인과 일직선이 되어 후킹력이 좋아진다.',
      '눈(eye)이 위로 향한 바늘보다 아래로 향한 바늘에 잘 맞는다.',
    ],
  },
  {
    slug: 'fg',
    ...
    difficulty: 'hard',
    strength: '줄 강도에 가깝게 유지되는 편(숙련도에 따라 다름)',
    tags: ['PE', '리더', '쇼크리더'],
    steps: [
      '엄지·검지로 리더를 PE와 약 45도 각도로 잡는다.',
  ...
  },
  {
    ...
    difficulty: 'medium',
    strength: '줄 강도 약 85% 유지(조건에 따라 다름)',
    tags: ['플라이', '티펫'],
    steps: [
      '두 줄을 X자로 교차시킨다.',
      '한쪽 줄 끝을 반대쪽 줄을 따라 5~7회 감는다.',
      '같은 방법으로 다른 줄도 반대 방향으로 5~7회 감는다.',
      '양쪽 줄 끝을 교차된 중앙 고리에 넣는다.',
      '네 가닥을 동시에 당겨 매듭 중앙으로 모은다.',
    ],
    tips: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    strength: '줄 강도 약 80~85% 유지(조건에 따라 다름)',
    featured: true,
    tags: ['PE', '모노', '플루오로'],
    steps: [
  ...
  },
  {
    ...
    difficulty: 'easy',
    strength: '줄 강도 약 75~80% 유지(조건에 따라 다름)',
    tags: ['빠른연결', '플라이', '임시'],
    steps: [
      '두 줄을 15cm 정도 겹쳐 놓는다.',
      '겹친 부분으로 고리를 하나 만든다.',
  ...
  },
  {
    ...
    difficulty: 'easy',
    strength: '줄 강도 약 80% 유지(조건에 따라 다름)',
    tags: ['루프', '드로퍼'],
    steps: [
      '줄을 접어 이중 고리를 만든다.',
  ...
  },
  {
    ...
    difficulty: 'medium',
    strength: '줄 강도 약 75% 유지(조건에 따라 다름)',
    tags: ['원투', '바다', '가지줄'],
    steps: [
...
export function getAdjacentKnots(slug: string): { prev?: Knot; next?: Knot } {
  const index = KNOTS.findIndex((k) => k.slug === slug);
  if (index < 0) return {};
  return {
    prev: index > 0 ? KNOTS[index - 1] : undefined,
    next: index < KNOTS.length - 1 ? KNOTS[index + 1] : undefined,
  };
}

export function getFeaturedKnots(limit = 4): Knot[] {
  const featured = KNOTS.filter((k) => k.featured);
  if (featured.length >= limit) return featured.slice(0, limit);
  return [...featured, ...KNOTS.filter((k) => !k.featured)].slice(0, limit);
}

export function sortKnots(knots: Knot[], sort: KnotSort): Knot[] {
  const list = [...knots];
  if (sort === 'difficulty-asc') {
    return list.sort(
      (a, b) =>
        KNOT_DIFFICULTY_ORDER[a.difficulty] - KNOT_DIFFICULTY_ORDER[b.difficulty] ||
  ...
  if (sort === 'difficulty-desc') {
    return list.sort(
      (a, b) =>
        KNOT_DIFFICULTY_ORDER[b.difficulty] - KNOT_DIFFICULTY_ORDER[a.difficulty] ||
        a.nameKo.localeCompare(b.nameKo, 'ko'),
    );
  }
...
export function filterKnots(
  knots: Knot[],
  options: { category?: KnotCategory | 'all'; query?: string },
): Knot[] {
  ...
  return knots.filter((knot) => {
    ...
    return (
      ...
      (knot.tags?.some((tag) => tag.toLowerCase().includes(q)) ?? false)
    );
  });
}
